...
public class KakaoApiUtil {
	...
	public static Map<String, Object> getVehiclePaths(PlaceDTO from, PlaceDTO to, List<PlaceDTO> waypoints,
		...
		if (waypoints != null && !waypoints.isEmpty()) {
			String waypointsParam = waypoints.stream().map(wp -> wp.getX() + "," + wp.getY())
		...
		HttpRequest request = HttpRequest.newBuilder().header("Authorization", "KakaoAK " + REST_API_KEY)
				.header("Content-Type", "application/json").uri(URI.create(url.toString())).GET().build();
		...
		if (!routes.isEmpty()) {
			...
			if (summary != null) {
	            ...
	            // summary가 null인 경우 처리
	            throw new IOException("API response does not contain summary information.");
	        }
		}else {
	        // routes가 비어있는 경우 처리
	        throw new IOException("API response does not contain any routes.");
	    }
